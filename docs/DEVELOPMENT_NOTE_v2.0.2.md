...
### 📅 2026-07-25: Elasticsearch custom HttpHeaders 순정화 및 wave-messaging 데이터베이스 datt 단일화 패치
* `a82761d` - **Elasticsearch custom HttpHeaders 순정화 및 wave-messaging 데이터베이스 datt 단일화 패치**
    * **작업 내용**:
        ...
        * `docker-compose.yml` 에서 `wave-messaging-service`가 바라보던 데이터베이스 명칭을 `datt_wave` 에서 `datt` 으로 일원화하여, 부팅 시 `FATAL: database "datt_wave" does not exist` 예외를 내던 인프라 격리 불일치 결함 해결.
