...
### 📅 2026-08-01: 고객 지원(서비스 문의 및 신고) 기능 및 어드민 통합 대시보드 구축

* `(git rev-parse --short HEAD)` - **고객 지원 전체 스택 구현 (서비스 문의 & 신고)**
    * **작업 내용**:
        * **Backend**: `support` 도메인을 신설하여 `ServiceInquiry` (개선 문의, 매장 등록 건의) 및 `Report` (매장, 리뷰 신고) 엔티티를 구현했습니다. `SupportController`(유저 생성 API)와 `AdminSupportController`(관리자 조회/처리 API)를 구축하고 `SecurityConfig`에서 관리자 권한을 설정했습니다.
        * **Frontend**: 유저가 손쉽게 접근할 수 있도록 글로벌 헤더에 "서비스 문의" 메뉴를 추가하고, 단일 입력창 폼 형태의 `app/support/page.tsx`를 구현했습니다.
        * **Frontend**: 매장 상세 페이지 상단과 리뷰 카드 아이템에 "🚨 신고" 버튼을 추가하여 부적절한 데이터를 즉각 신고할 수 있도록 `window.prompt` 모달과 연동했습니다.
        * **Admin**: `app/admin/support/page.tsx`를 신설하여 접수된 서비스 문의와 신고 내역을 탭 형태로 분류하여 열람하고 '처리(RESOLVED)'할 수 있는 통합 대시보드를 구축했습니다.
    * **결과**: 사용자의 피드백과 서비스 건전성 관리를 일원화된 어드민 페이지에서 신속하게 대응할 수 있는 체계가 완성되었습니다.

### 📅 2026-07-28: 워크스페이스 채팅 연동 버그 픽스 및 구글 캘린더 스타일 UI 개편

* `(git rev-parse --short HEAD)` - **워크스페이스 기본 채팅방 개설 및 자동 참가 연동 버그 수정**
    * **작업 내용**:
        * 도메인 분리 이후 `WorkspaceService`에서 워크스페이스 개설 및 참가 시 `wave-messaging-service`로 기본 채팅방 개설/참가 API를 호출하지 않던 버그를 수정했습니다.
        * `RestTemplate`을 사용하여 내부망(8081 포트)으로 `POST /api/chat/rooms` 및 `POST /api/chat/rooms/{roomId}/join` 요청을 전송하도록 연동 로직을 추가했습니다.
    * **결과**: "연결된 대화방이 없습니다" 에러가 해결되고, 워크스페이스 가입 즉시 첫 채팅방에 입장할 수 있게 되었습니다.

* `(git rev-parse --short HEAD)` - **구글 캘린더 스타일의 워크스페이스 약속 캘린더 UI 전면 개편**
    * **작업 내용**:
        * 서드파티 라이브러리 없이 Tailwind CSS Grid(`grid-cols-7`)와 네이티브 JavaScript `Date` 객체만으로 완전한 커스텀 월간 캘린더를 렌더링했습니다.
        * 약속 데이터(`appointments`)를 날짜별로 그룹핑하여 각 날짜 셀에 시간 순으로 컬러 뱃지로 렌더링했습니다.
        * 날짜 빈칸 클릭 시 해당 날짜로 폼이 프리필(Pre-fill)된 '새 일정 등록' 모달, 뱃지 클릭 시 '상세 보기 및 삭제' 모달이 팝업되도록 인터랙션을 고도화했습니다.
    * **결과**: 사용자가 구글 캘린더처럼 직관적으로 팀 일정을 파악하고 관리할 수 있도록 UI/UX가 비약적으로 상승했습니다.
...
* **�۾� ����**: ����ڰ� �����ʿ��� �ڽ��� ���� ���Ǹ� Ȯ���ϰ�, ���� �ۼ� ������ �̵� ���Ǽ��� �����ϸ�, ������ ȭ���� ���μ��� ������.
* **�۾� ����**:
  * **[Backend]** /api/support/inquiries/me ��������Ʈ�� �߰��Ͽ� �α����� ������� ���� ������ ����¡ ��ȸ�ϵ��� ����.
  * **[Frontend]** ����������(pp/my/profile/page.tsx)�� '�� ���� ����' ���� �߰��ϰ� �ۼ��� ���� ������ ó�� ���� ǥ��.
  * **[Frontend]** ���� �ۼ� ������(pp/support/page.tsx)�� �ڷΰ��� ��ư(ArrowLeft) �߰�.
  * **[Frontend]** ������ ��� ���� ������(pp/admin/support/page.tsx)�� ���, �ؽ�Ʈ ������ ��ũ �׸�(slate-900 ��)�� �����Ͽ� ��ü ���� ���̾ƿ�� ������ ����.


### 37d4a8d SupportController ���� ��ü ĳ���� ������ ���� ���������� �����̷�Ʈ ���� ����
* **�۾� ����**: spring-boot-app/src/main/java/xyz/datt/domain/support/controller/SupportController.java
* **�۾� ����**: ���� ���� ������ �ҷ����� API ȣ�� ��, ���� ��ü(CustomUserDetails)�� String���� ĳ�����Ϸ��� �߻��ϴ� ����(null �Ҵ�)�� ���� 401 ���� �� �ڵ� �α׾ƿ��� �߻��ϴ� ���� ����
* **�۾� ����**: @AuthenticationPrincipal�� Ÿ���� CustomUserDetails�� �����ϰ� String.valueOf(userDetails.getMemberId())�� ����Ͽ� �����ϰ� userId�� �����ϵ��� ����.


### 2026-08-01 �˸� ��� �� ���� �亯 ��� ����
* **�۾� ����**: ����ڰ� ���� ���� �� ���� ���� �� �������� �亯 �ۼ� �����ϵ��� ����. ���Ҿ� �˸� �ý��� �ű� ����
* **�۾� ����**: 
  1. Frontend: ���� ���� ���� �� myInquiries ĳ�� ��ȿȭ (app/support/page.tsx)
  2. Backend: ServiceInquiry ��ƼƼ�� answer �ʵ� �߰� �� SupportService, AdminSupportController ������Ʈ
  3. Backend: Notification ������(Entity, Repository, Service, Controller) �ű� �ۼ� �� ���� ó�� �� �˸� ����
  4. Frontend: Admin ������(app/admin/support/page.tsx)���� �亯 ������Ʈ �ۼ� ���� �߰�
  5. Frontend: ����������(app/my/profile/page.tsx)���� �亯(answer) ��ȸ UI ������ �߰�
  6. Frontend: GlobalHeader�� NotificationDropdown UI ���� �� �˸� ���� ó�� ���(hooks/useNotifications.ts) ����


### 2026-08-01 UI ��Ī ���� �� �׺���̼� ��� ����
* **�۾� ����**: ���� ��Ī ����(��ũ�����̽� -> ũ��) �� Ž�� ���Ǽ� ���븦 ���� �޴� �׷�ȭ
* **�۾� ����**: 
  1. Frontend: ���� �� ��� '��ũ�����̽�' �ؽ�Ʈ�� 'ũ��'�� ġȯ (app/workspaces, GlobalHeader ��)
  2. Frontend: ��� GlobalHeader �� �ϴ� MobileNav�� �޴����� '���Ž��'�� '��ġŽ��'�� 'Ž��'�̶�� �ϳ��� ��Ӵٿ�(�˾�) �޴��� �����ϰ� ù ��° ������ ��ġ
...
## 6624f53
- 작업 목적: 배포 스크립트(.github/workflows/deploy.yml) VAPID 키 주입 추가
- 구체적 내용: Github Secrets에서 VAPID 키를 읽어 서버의 .env 파일에 주입하도록 수정


## [TEMP_HASH]
- �۾� ����: � ���� ����Ʈ���� ȯ�溯�� ���� ���� �ذ�
- ��ü�� ����: next-js-app Dockerfile �� docker-compose.yml�� NEXT_PUBLIC_VAPID_PUBLIC_KEY ȯ�溯�� ���� �߰�
