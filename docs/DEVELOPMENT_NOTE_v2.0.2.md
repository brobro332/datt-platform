...
### 📅 2026-07-25: Elasticsearch 8.17.0 다운그레이드, 빈 충돌 및 런타임 호환 수동 빈 오버라이딩 최종 패치
* `700fce6` - **Elasticsearch 8.17.0 다운그레이드, 빈 충돌 및 런타임 호환 수동 빈 오버라이딩 최종 패치**
    * **작업 내용**:
        ...
        * `MyElasticsearchConfig.java`에서 선언한 `restClient` 빈 이름이 `RestClientConfig.java`의 Spring Web `RestClient` 빈 이름과 충돌하는 예외(`already been defined and overriding is disabled`)를 우회하고자, ES용 빈 이름을 `elasticsearchRestClient`로 교체 리팩토링하여 부팅 안정성 확보.
