...
### 📅 2026-07-25: Elasticsearch 8.17.0 다운그레이드 및 런타임 호환 수동 빈 오버라이딩 최종 패치
* `1a875a0` - **Elasticsearch 8.17.0 다운그레이드 및 런타임 호환 수동 빈 오버라이딩 최종 패치**
    * **작업 내용**:
        ...
        * 이 과정에서 스프링 부트 4.x 자동구성 시 9.x 전용 클래스(`Rest5ClientOptions`)를 호출하려다 런타임에 `NoClassDefFoundError`가 터지던 기동장애를 우회하기 위해, `MyElasticsearchConfig.java`에서 `ElasticsearchConfiguration` 상속을 배제하고 `RestClient`, `ElasticsearchTransport`, `ElasticsearchClient`, `ElasticsearchOperations` 빈을 순수 8.x 규격에 맞추어 직접 수동으로 선언 오버라이딩하여 기동에 성공.
