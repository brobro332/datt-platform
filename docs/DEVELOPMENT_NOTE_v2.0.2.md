...
### 📅 2026-07-25: Elasticsearch 기동 HEAD 요청 예외 회피 및 HttpHeaders set() 덮어쓰기 & Content-Type 제거 패치
* `a19c58e` - **Elasticsearch 기동 HEAD 요청 예외 회피 및 HttpHeaders set() 덮어쓰기 & Content-Type 제거 패치**
    * **작업 내용**:
        * `MyElasticsearchConfig.java`의 API 요청 헤더 바인딩 시 `add()` 대신 `set()` 메소드를 사용하여 스프링 프레임워크가 자동 삽입하던 호환성 헤더(`compatible-with=9`)를 완전히 제거(Override)하고 단일 미디어 타입 `application/json`만 전달하도록 수정.
        * **[추가 조치]** HTTP Body가 없는 `indices.refresh` 요청에서 `Content-Type` 헤더가 강제 주입되던 현상을 수정하고자 `Content-Type` 수동 주입 구문을 영구 제거하여 `media_type_header_exception` 에러 완전 차단.
