...
* `89a13bc` - **위치탐색 모바일 레이아웃 본래 앱 구조 완전 복원 및 모바일 overflow 해제**
    * **작업 내용**:
        * 위치탐색 화면(`/map`)을 인위적인 카드 보드가 아닌 본래의 웅장하고 미려했던 모바일 앱 UX 구조로 완전 복원.
        * 모바일 화면 컨테이너의 `overflow-hidden`을 `lg:overflow-hidden`으로 한 줄 미세 수정하여 모바일 화면에서 원래처럼 사이드바 목록과 지도가 찌그러짐 없이 당당하고 시원하게 렌더링되도록 교정.
* `16c7845` - **Next.js 루트 레이아웃 Viewport Meta 지정 및 모바일 네이티브 앱 UX/하단 네비게이션 보장**
    * **작업 내용**:
        * `app/layout.tsx`에 `Viewport` 메타 설정(`width: "device-width", initialScale: 1`)을 명시하여 모바일 기기 접속 시 브라우저가 데스크톱 뷰포트로 축소 오인하는 현상 원천 차단.
        * 모바일 기기 접속 시 하단 모바일 전용 네비게이션 바가 감춰지지 않고 모바일 앱 느낌 그대로 선명하게 출력되도록 뷰포트 락인.
