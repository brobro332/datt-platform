...
* `7a05a9e` - **위치탐색 모바일 전용 지도로 보기 / 목록으로 보기 뷰 전환 스위처 탑재 및 UX 전면 개편**
    * **작업 내용**:
        * 모바일 화면(`lg` 미만)에서 지도와 목록이 세로로 겹쳐 지도가 짤리거나 탐색 스크롤과 지도 터치가 뒤엉키는 문제 해결을 위해 모바일 전용 뷰 전환 스위처(`mobileTab: "map" | "list"`) 탑재.
        * `지도로 보기` 선택 시 모바일 화면 전체 높이(`h-[calc(100vh-230px)] min-h-[480px]`)로 카카오 지도를 꽉 채워 렌더링하고, 장소 마커 클릭 시 하단 바텀시트 정보 카드가 노출되도록 사용성 개편.
        * `목록으로 보기` 선택 시 검색창, 내 위치 버튼, 정렬 탭 및 장소 목록을 쾌적하게 훑어볼 수 있도록 뷰포트 분리 구현.
