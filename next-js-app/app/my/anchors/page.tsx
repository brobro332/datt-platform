...
import { Anchor, MapPin, Eye, Heart, ListPlus } from "lucide-react";

const PAGE_SIZE = 6;

export default function MyAnchorsPage() {
  const [page, setPage] = useState(0);
  const { data, isLoading, isError } = useMyAnchors({ sortType: "LATEST", page, size: PAGE_SIZE });
    ...
    <MainLayout requireAuth>
      <section className="space-y-8 pb-32">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-100">
            <div>
              <p className="text-xs font-semibold text-blue-600 uppercase tracking-widest flex items-center gap-1">
                <Anchor className="w-3.5 h-3.5 text-blue-500" /> My Anchors
              </p>
              <h1 className="mt-1 text-3xl font-black tracking-tight text-slate-900">생성한 닻</h1>
              <p className="mt-2 text-sm leading-relaxed text-slate-500 max-w-xl font-semibold">
                내가 지정한 중심 구역들을 묶어 큐레이션한 나만의 닻 목록입니다. 각 닻의 상세 추천 리포트와 매핑 코스를 확인해 보세요.
              </p>
            </div>
            <Link href="/place-master" className="shrink-0">
              <Button size="md" className="rounded-xl shadow-md flex items-center gap-1.5">
                <ListPlus className="w-4 h-4" /> 새 닻 내리기
              </Button>
            </Link>
          </div>
        </div>
        ...
        <AsyncStateView
          ...
          loadingMessage="내 닻 목록을 불러오는 중입니다..."
          errorTitle="닻 목록 조회 실패"
          errorMessage="목록 정보를 불러오지 못했습니다. 다시 시도해 주세요."
          emptyTitle="생성한 닻이 없습니다."
          emptyDescription="대표 구역을 설정하여 첫 닻을 정박해 보세요!"
        >
          {data && (
                      ...
                      <div className="flex items-center justify-between border-t border-slate-100/80 pt-3 text-[10px] font-black text-slate-400">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="flex items-center gap-0.5 text-rose-500">
                            <Heart className="w-3 h-3 fill-rose-500 text-rose-500" /> {anchor.likeCount.toLocaleString()}
                          </span>
                        </div>
