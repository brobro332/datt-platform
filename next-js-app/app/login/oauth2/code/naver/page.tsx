...
import { loginSocialNaver } from "@/services/authService";
...
function NaverCallbackContent() {
  ...
  useEffect(() => {
    ...
    async function handleLogin(authCode: string) {
      try {
        const response = await loginSocialNaver(authCode);
        ...
        console.error(err);
        setError("네이버 소셜 로그인에 실패했습니다. 다시 시도해 주세요.");
      }
    }
    ...
    return (
      <div className="flex min-h-[65vh] flex-col items-center justify-center p-4">
        <div className="w-full max-w-md bg-white/90 backdrop-blur-md rounded-[2.5rem] border border-slate-200/50 p-8 md:p-10 shadow-[0_30px_70px_rgba(0,0,0,0.06)] relative overflow-hidden animate-in fade-in zoom-in-95 duration-300">
          {/* Subtle Naver theme header accent line */}
          <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-green-400 via-green-300 to-emerald-400" />
          
          {/* Ambient background glows */}
          <div className="absolute -top-10 -left-10 w-28 h-28 rounded-full bg-green-500/5 blur-2xl pointer-events-none" />
          
          <div className="flex flex-col items-center text-center space-y-4 mb-8">
            {/* Dynamic Avatar Preview */}
            <div className="relative">
              <div className="absolute inset-0 rounded-full bg-gradient-to-tr from-green-400 to-emerald-400 blur-sm opacity-40 animate-pulse" />
              <div className="relative h-24 w-24 rounded-full bg-gradient-to-tr from-green-400 via-emerald-400 to-green-300 border border-white flex items-center justify-center text-4xl font-black text-white shadow-md transition-transform duration-300 transform hover:scale-105">
                {avatarLetter}
              </div>
            </div>
          ...
          <form onSubmit={handleRegisterNickname} className="space-y-6">
            <div className="space-y-2">
              ...
              <input
                ...
                maxLength={20}
                className="w-full px-5 py-4 text-base font-black rounded-2xl border border-slate-200 bg-slate-50/50 text-slate-900 focus:outline-none focus:ring-2 focus:ring-green-400/50 focus:bg-white transition-all shadow-inner placeholder-slate-350"
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full py-4 rounded-2xl bg-[#03C75A] hover:bg-[#03C75A]/95 text-white font-black text-sm transition-all duration-200 shadow-md shadow-[#03C75A]/15 active:scale-98 disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2"
            >
    ...
    <div className="flex min-h-[50vh] flex-col items-center justify-center space-y-4">
      {error ? (
        ...
        <div className="flex flex-col items-center space-y-3.5">
          ...
          <p className="text-sm font-black text-slate-700 animate-pulse">네이버 로그인 처리 중입니다...</p>
        </div>
      )}
    </div>
  );
